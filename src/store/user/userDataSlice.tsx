    ...
    if (token) {
      ...
      switch (form.platform) {
        case IntegrationPlatform.Notion: {
          try {
            const notionAuthResponse = await fetchNotionAccessToken(
              form.temp_code
            );
            ...
            return rejectWithValue(
              "Error occurred while trying to add integration"
            );
          }
        }
        case IntegrationPlatform.Google: {
          try {
            ...
            const googleTokenResponse = await obtainGoogleAccessToken(
              form.temp_code
            );
            ...
            const requestedScope = scopes.filter(
              (s: string) => s !== Scope.UserInfo && s !== Scope.openId
            );
            ...
            const googIntegs = integrations.filter(
              (i) => i.platform === IntegrationPlatform.Google
            );
            const existingIntegration = googIntegs.find(
              (i) => i.email === email
            );
            ...
            if (
              existingIntegration &&
              existingIntegration.scopes.includes(requestedScope[0])
            ) {
            ...
            const newIntegration: AddIntegrationRequest = {
              ...
              scopes:
                existingIntegration === undefined
                  ? [...requestedScope]
                  : existingIntegration.scopes.concat(requestedScope),
              access_token: access_token,
              extra: extra,
            };

            // TODO: NEED TO ASSERT OK
            const integration = await addIntegrationForUser(
              token,
              newIntegration
            );
...
// SELECTORS
export const userDataFetchStatusSelector = (state: RootState) =>
...
export const userDataIntegrationByPlatformSelector = (
  ...
  createSelector(userDataIntegrationsSelector, (integrations: Integration[]) =>
