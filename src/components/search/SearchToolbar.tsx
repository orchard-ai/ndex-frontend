...
const SearchToolbar = () => {
  ...
  const defaultConnectionFilter: ConnectionFilter[] = connections.map(
    (connection, index) => {
      return {
        id: index,
  ...
  const [connectionFilters, setConnectionFilter]: [
    ConnectionFilter[],
    (value: ConnectionFilter[]) => void
  ...
  return (
    <div className="flex w-full p-4 space-x-4 ">
      <p className="align-middle my-auto text-ndex-text-grey">
        {" "}
        searching in:{" "}
      </p>
      <button className="flex space-x-4 p-1">
        {connectionFilters.map((connection, index) => {
          return (
            <div
              key={index}
              className={`rounded-md p-1 active:bg-ndex-search-toolbar-icon-active
                            ${
                              connection.selected
                                ? "bg-ndex-search-toolbar-icon-selected"
                                : "hover:bg-ndex-search-toolbar-icon-hover"
                            }
                        `}
